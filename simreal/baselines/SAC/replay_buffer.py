...
class ReplayBuffer(object):
    def __init__(self, env, capacity, device):
        ...
        self.param_list = np.zeros([capacity, len(env.params)], dtype=np.float32)
    ...
    def _add(self, observation, action, reward, next_observation, terminal, params):
        ...
        self.param_list[self.pointer] = list(params.values())
    ...
    def add(self, traj):
        for t in range(1, traj.T+1):
            self._add(traj[t-1].observation, traj.actions[t-1], traj[t].reward, traj[t].observation, traj[t].terminal(), traj[t].params)
