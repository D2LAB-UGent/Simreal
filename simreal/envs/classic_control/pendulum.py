...
class PendulumEnv(gym.Env):
    ...
    def step(self,u):
        ...
        # print('Used params: ', m, l)

        u = np.clip(u, -self.max_torque, self.max_torque)[0]
        self.last_u = u # for rendering
        costs = angle_normalize(th)**2 + .1*thdot**2 + .001*(u**2)

        newthdot = thdot + (-3*g/(2*l) * np.sin(th + np.pi) + 3./(m*l**2)*(u-self.friction*thdot)) * dt     #joint friction added
    ...
    def render(self, mode='human'):
        ...
        rod = rendering.make_capsule(self.l, .2 * self.m)
        rod.set_color(.8, .3, .3)
